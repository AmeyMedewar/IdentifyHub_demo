...
database = FaceDatabase("face_database.pkl")  # Make sure this file is in the same folder
...
async def identify_person(image: UploadFile = File(...)):
    try:
        ...
        logger.info(
            "identify_person: pil_image size=%s mode=%s",
            pil_image.size,
            pil_image.mode
        )
...
async def add_person_multiple(
    name: str = Form(..., description="Name of the person"),
    images: list[UploadFile] = File(..., description="Multiple face images of the person")
):
    """
    ...
    try:
        ...
        if embeddings_added == 0:
            response_data["message"] = "Failed to process any images. No faces were detected."
