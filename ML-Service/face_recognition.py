...
class FaceRecognizer:
    ...
    def __init__(self, use_gpu: bool = False, model_name: str = "ArcFace"):
        ...
        try:
            ...
            print(
                f"Model initialized successfully (Model: {self.model_name}, GPU: {use_gpu})"
            )
        ...
        for name, embeddings_list in self.embeddings.items():
            for stored_embedding in embeddings_list:
                if metric == "cosine":
                    ...
                    score = FaceRecognizer.cosine_similarity(
                    ...
                    score = FaceRecognizer.euclidean_distance(
                        query_embedding, stored_embedding
                    )
                    if score < best_score:
                        best_score = score
...
def load_images_from_folder(
    folder_path: str, recognizer: FaceRecognizer
    ...
    for person_folder in folder.iterdir():
        ...
        print(
            f"  Total embeddings for {person_name}: {len(embeddings_dict[person_name])}"
        )
